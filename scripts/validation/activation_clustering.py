...
def _acts_for_class(data, params, target_class, extra_poison=None):
    ...
    m = MLP(n_features=data.n_features)
    m.set_params(params)
...
def main() -> None:
    ...
    attack_pool = data.X_train[data.y_train == ATTACK]
    ...
    with open(os.path.join(RESULTS_DIR, "task0_3_activation_clustering.json"), "w") as f:
        json.dump(out, f, indent=2)
    print("\nwrote results/validation/task0_3_activation_clustering.json")
