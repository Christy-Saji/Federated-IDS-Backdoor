...
def main():
    ...
    print("-" * (width + 62))
    for r in sorted(rows, key=lambda r: (r["aggregator"], r["seed"])):
    ...
    # Scoring the attackers and removing them are different claims. This is the
    # second one, which is the one an operator actually cares about.
    ...
    for r in rows:
        by_agg.setdefault(r["aggregator"], []).append(r)
    multi = {a: rs for a, rs in by_agg.items() if len(rs) > 1}
    if multi:
        print(f"\n{'aggregator':{width}s} {'seeds':>6s} {'raw AUC mean+-std':>20s} "
              f"{'orientations':>26s}")
        print("-" * (width + 56))
        for a, rs in sorted(multi.items()):
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Malicious-client detection over time (real CIC-IDS2017)")
    ax.legend(fontsize=8, loc="lower right")
    fig.tight_layout()
    path = os.path.join(FIGURES, "detection_by_round.png")
